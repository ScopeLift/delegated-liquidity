...
import {BaseFactory} from "src/BaseFactory.sol";
...
contract CounterHook is BaseHook, IHookFeeManager {
  ...
  /// @dev This callback is only made if the Fee.HOOK_SWAP_FEE_FLAG or Fee.HOOK_WITHDRAW_FEE_FLAG in
  /// set in the pool's key.fee.
...
contract CounterFactory is BaseFactory {
  ...
  {
    bytecodeHash =
      keccak256(abi.encodePacked(type(CounterHook).creationCode, abi.encode(poolManager)));
  }
}
