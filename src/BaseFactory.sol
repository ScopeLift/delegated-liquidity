...
abstract contract BaseFactory {
  ...
  address public immutable TARGET_PREFIX;

  constructor(address _targetPrefix) {
    TARGET_PREFIX = _targetPrefix;
  }
  ...
  function _computeHookAddress(IPoolManager poolManager, bytes32 salt)
    internal
    view
    returns (address)
  {
    bytes32 hash =
      keccak256(abi.encodePacked(bytes1(0xff), address(this), salt, _hashBytecode(poolManager)));
    return address(uint160(uint256(hash)));
  }

  /// @dev The implementing contract must override this function to return the bytecode hash of its
  /// contract
  /// For example, the CounterHook contract would return:
  /// bytecodeHash = keccak256(abi.encodePacked(type(CounterHook).creationCode,
  /// abi.encode(poolManager)));
  function _hashBytecode(IPoolManager poolManager)
    internal
    pure
    virtual
  ...
  function _isPrefix(address _address) internal view returns (bool) {
    ...
    return actualPrefix == TARGET_PREFIX;
  }
}
