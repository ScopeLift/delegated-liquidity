...
library HookMiner {
  ...
  /// contract `address(this)` or the pranking address
  ///                 In `forge script`, this should be `0x4e59b44847b379578588920cA78FbF26c0B4956C`
  /// (CREATE2 Deployer Proxy)
  /// @param flags The desired flags for the hook address
  /// @param seed Use 0 for as a default. An optional starting salt when linearly searching for a
  /// salt. Useful for finding salts for multiple hooks with the same flags
  ...
  function find(
    ...
    for (salt; salt < MAX_LOOP;) {
      ...
      if (uint160(hookAddress) & FLAG_MASK == flags) return (hookAddress, bytes32(salt));
